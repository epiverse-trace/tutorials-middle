...
To estimate key metrics of transmissibility using case data we must account for delays between the date of infections and date of reported cases. In an outbreak situation, data are often available on reported dates only, therefore we must use estimation methods which account for these delays. 
...
When specifying a distribution, it is useful to visualise the probability density to see the peak and spread of the distribution, in this case a Lognormal distribution. We can use the functions `convert_to_logmean()` and `convert_to_logsd()` to  convert the mean and standard deviation of a normal distribution to that of a Lognormal distribution. 
...
The cases are `r summary(estimates)$estimate[summary(estimates)$measure=="New confirmed cases by infection date"]`


#### `Expected change in daily cases` 

`r summary(estimates)$estimate[summary(estimates)$measure=="Expected change in daily cases"]`


#### `Effective reproduction no.`
`r summary(estimates)$estimate[summary(estimates)$measure=="Effective reproduction no."]`
...
`r summary(estimates)$estimate[summary(estimates)$measure=="Rate of growth"]`
...
`r summary(estimates)$estimate[summary(estimates)$measure=="Doubling/halving time (days)"]`
...
#reported_regional_cases <- regional_cases
reported_regional_cases <- regional_cases[1:10,]
