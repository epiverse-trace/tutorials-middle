...
```{r,warning=FALSE,message=FALSE}
incubation_period_variable <- Gamma(
  mean = Normal(mean = 4,sd = 0.5),
...
```{r,warning=FALSE,message=FALSE}
reporting_delay_variable <- LogNormal(
  mean = Normal(mean = log_mean, sd = 0.5),
  sd = Normal(mean = log_sd, sd = 0.5),
  max = 10
)
```
...
```{r,warning=FALSE,message=FALSE}
generation_time_variable <- LogNormal(
