...
In this tutorial we are going to learn how to use the `{EpiNow2}` package to estimate the time-varying reproduction number. We'll get input data from `{incidence2}`. We’ll use the `{dplyr}` package to arrange some of its outputs, `{ggplot2}` to visualize case distribution, and the pipe `%>%` to connect some of their functions, so let’s also call to the `{tidyverse}` package:
...
df <- data.frame(
  x = x,
  density = dlnorm(x, meanlog = log_mean, sdlog = log_sd)
)
...
reporting_delay_variable <- EpiNow2::LogNormal(
  mean = EpiNow2::Normal(mean = log_mean, sd = 0.5),
  sd = EpiNow2::Normal(mean = log_sd, sd = 0.5),
  max = 10
)
```
...
rt <- EpiNow2::rt_opts(prior = list(mean = rt_log_mean, sd = rt_log_sd))
```
