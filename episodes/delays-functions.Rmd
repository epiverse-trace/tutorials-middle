...
This episode will integrate the content of the two previous episodes. Let's start by loading the `{epiparameter}` and `{EpiNow2}` package. For `{EpiNow2}`, we'll set 4 cores to be used in parallel computations. We'll use the pipe `%>%`, some `{dplyr}` verbs and `{ggplot2}`, so let's also call to the `{tidyverse}` package:
...
Now, we have an epidemiological parameter we can use in our analysis! For example, to quantify transmission, in the chunk below we replaced one of the **summary statistics** inputs into `EpiNow2::LogNormal()`:
