...
Disease severity is measured in terms of case fatality ratio ($cfr$) --total number of deaths $D$ divided by total number of confirmed cases $C$. We can refer to the crude or _naive CFR_ as $nCFR$:
...
To calculate the $nCFR$, the `cfr` package requires a data frame with three columns named:
...
When we use `cfr_static()` to `data` only, we calculate the naive CFR ($nCFR$):
...
## Early determination of an unbiased CFR
...
For calculation of the factor of underestimation $u_{t}$, two pieces of information are needed: the incidence function $c_{t}$ and the distribution of time from onset to death $f_{s}$. For ct, we use the published dates of onset among confirmed cases, while fs is assumed known.
