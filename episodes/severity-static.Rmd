...
- Why adjust a CFR with a delay distribution?

- When should we estimate the CFR in an ongoing outbreak?
...
- Does the virus have the potential to cause a devastating pandemic?
...
One epidemiological approach to estimate the clinical severity is quantifying the Case Fatality Ratio or CFR. We define CFR as the ratio of deaths from an infectious disease to the number of confirmed diagnosed cases. However, calculating this directly from the cumulative number of deaths to cases during the course of an epidemic tends to result in a biased CFR given the time delay from onset to death, varying substantially as the epidemic progresses and stabilising at the later stages of the outbreak ([Ghani et al., 2005](https://academic.oup.com/aje/article/162/5/479/82647?login=false#620743)).

![Observed biased confirmed case fatality ratio (CFR) estimates as a function of time (thick line) calculated as the ratio of the cumulative number of confirmed cases to deaths at time t. The estimate at the end of an outbreak (i.e. 302/1755 = 17.2 %) is the realised cCFR by the end of the epidemic. The horizontal continuous line and dotted lines show the expected value and the 95% confidence intervals (95% CI) of the predicted delay-adjusted cCFR estimate (based on Nishiura et al., 2009) only by using the observed data until 27 Mar 2003 (estimated at 18.1 % (95% CI: 10.5, 28.1).](fig/cfr-pone.0006852.g003-fig_c.png)
...
We measure disease severity in terms of case fatality ratio (CFR) --which is given by the ratio of the cumulative number of deaths $D_{t}$ to the cumulative number of confirmed cases $C_{t}$ at a certain time $t$. We can refer to the _naive CFR_ (also crude or biased CFR, $b_{t}$):
...
Improving an _early_ epidemiological assessment of an delay-adjusted CFR is crucial for determining virulence, shaping the level and choices of public health intervention, and providing advice to the general public. 
...
- What were the main sources of bias?
...
Nevertheless, even by only using the observed data for the period 19 March to 2 April, `cfr_static()` can yield an appropriate prediction (Figure D), e.g. the delay-adjusted CFR at 27 Mar is 18.1 % (95% CI: 10.5, 28.1). An overestimation is seen in the very early stages of the epidemic, but the 95% confidence limits in the later stages include the realised CFR (i.e. 17.2 %).

![Early determination of the delay-adjusted confirmed case fatality ratio of severe acute respiratory syndrome (SARS) in Hong Kong, 2003.](fig/cfr-pone.0006852.g003-fig_d.png)
...
We can use either visual inspection or analysing the output data frames.
...
`{cfr}` have a function called `cfr_time_varying()` that have a different functionallity that make them not comparable.

#### `cfr_rolling()`

`cfr_rolling()` shows the estimated CFR on each day of the outbreak, given that future data on cases and deaths is unavailable at the time. The final value of `cfr_rolling()` estimates is identical to `cfr_static()` on the same data.

Remember, as shown above, `cfr_rolling()` is helpful to get early-stage CFR estimates and check whether an outbreak's CFR estimate has stabilised. Thus, `cfr_rolling()` is not sensible to the length or size of the epidemic.
...
Use the `incidence2::covidregionaldataUK` data frame to estimate a delay-adjusted CFR stratified by region.
...
- You can read the `{cfr}` vignette on [Handling data from `{incidence2}`](https://epiverse-trace.github.io/cfr/articles/data_from_incidence2.html)
...
Great! But, how can we interpret that there is a regional variability of severity from the same diagnosed pathogen?
...
If we change the inputs for the numerator (`cases`) and denominator (`deaths`), we can use `{cfr}` to estimate more severity measures like the Infection fatality risk (IFR) or the Hospitalisation Fatality Risk (HFR). We can follow this analogy: 
