...
```{r}
# Calculate the static CFR without adjusting for delays for the first 30 days
...
```{r}
# Calculate the static CFR while adjusting for delays for the first 30 days
cfr_static(
  data = ebola1976[1:30,],
...
On the challenge above, we discovered that the difference between the _naive_ and _delay-adjusted_ CFR estimates using `cfr_static()` are different. **Why do they differ?**
...
The `cfr_static()` estimate is useful to get an overall severity estimate of the outbreak (so far). Once the outbreak has ended or has progressed such that more deaths are reported, the estimated CFR is then _closest to_ the 'true' unbiased CFR.
...
```{r}
# Calculate the rolling daily naive CFR for all the 73 days in the Ebola dataset
...
mers_incidence %>% 
  prepare_data(cases_variable = "dt_onset",deaths_variable = "dt_death")
