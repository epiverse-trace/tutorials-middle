...
The naive CFR estimates the overall disease severity, given the _latest data available at the moment_, as `r outn_estimate` with a 95% confidence interval between `r outn_low` and `r outn_high`.
...
The naive CFR estimates the overall disease severity, given the _latest data available at the moment_, as `r outc_estimate` with a 95% confidence interval between `r outc_low` and `r outc_high`.
...
#### First, the expected number of outcomes at day i
...
- $c_0 \times (f_1)$: the number of observed cases on day 0 times the density at day 1, plus
...
- $c_0 \times (f_2)$: the number of observed cases on day 0 times the density at day 2, plus
- $c_1 \times (f_1)$: the number of observed cases on day 1 times the density at day 1, plus
...
### Second, the total expected number of outcomes "by" time t
...
- $c_0 \times (f_0 + f_1 + f_2)$: expected outcomes from day 0's cases, by day 2
- $c_1 \times (f_0 + f_1)$: expected outcomes from day 1's cases, by day 2
- $c_2 \times (f_0)$: expected outcomes from day 2's cases, by day 2

Summing these terms gives the total expected outcomes by day 2. This is exactly the double summation shown above, now worked out term by term. 

This total is used to calculate the **underestimation factor**, the fraction of cases whose outcomes are already known:

$$
u_t = \frac{\text{cumsum}(\text{expected outcomes})_t}{\text{cumsum}(\text{cases})_t}
$$

A low `u_t` means many cases are still unresolved, which drags the naive CFR down. We can see this in the last row of `cfr::estimate_outcomes()`:
...
cfr::estimate_outcomes(
  ...
  dplyr::mutate(
    u_t_manual = cumsum(estimated_outcomes) / cumsum(cases)
  ) %>% 
  slice_tail(n = 1)
```

At day 30, with a $u_t = 0.499$, only about half of the expected outcomes have become known so far. As a result, the naive CFR ($0.474$), based only on currently known deaths, substantially underestimates the delay-adjusted CFR ($0.950$), landing at just 49.9% of its value.
