...
These two are also common questions at the early stage of an epidemic:
...
Load the `ebola_kikwit_1995` from `{outbreaks}` and estimate the adjusted CFR using the appropriate distribution delay
...
For less severe emerging pathogens, the case definition typically only encompasses a small fraction of all infected individuals, and hence the infection fatality rate (i.e. the proportion of infected individuals who die, rather than the proportion of cases who die—as per the case definition, which may not be equivalent to infection) may be a more useful measure of severity 
...
How would you estimate severity heterogeneity?

::::::::::::::::: hint

use `{incidence2}` to group aggregated case data by the `region` variable.
