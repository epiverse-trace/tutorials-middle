...
simulated_chains_map %>%
  dplyr::as_tibble() %>%
  dplyr::count(chain_id, sort = TRUE)
```
...
simulated_chains_map %>%
  dplyr::filter(chain_id == chain_to_observe)
```
...
threshhold_summary <- sim_chains_max %>%
  ...
  dplyr::mutate(chains_number = number_chains) %>%
