...
However, how can we have an early understanding of a disease epidemic potential from the mean reproduction number $R$ and dispersion $k$ of an offspring distribution?

<!-- what we are going to do -->

In this episode we are going to use the `{epichains}` package to simulate transmission chains to estimate the potential for explosive outbreaks. We are going to use it with functions from `{epiparameter}`, `{dplyr}` and `{purrr}`, so also loading the `{tidyverse}` package:
...
The *size* calculation includes the first case and the *length* calculation includes the first generation when the first case starts the chain (See figure below).
...
First, let's make a minimal sketch of how we use `purrr::map()` with `epichains::simulate_chains()`. The `map()` function requires two arguments:
...
Chain `r chain_to_observe` is telling us an **story**: "In the first transmission generation at `time = 0`, one index case infected the first case with `sim_id = 1`. Then, in the second transmission generation (between `time` 10 to 16), `sim_id = 1` infected five cases. Later, in the third transmission generation (between `time` 26 to 30), `sim_id = 2` infected three new cases."
...
Notice that when we increase the dispersion parameter from $k = 0.01$ to $k = \infty$, given a fixed reproduction number $R = 1.5$, the proportion of simulated outbreaks that reached the 100-case threshold increases.
