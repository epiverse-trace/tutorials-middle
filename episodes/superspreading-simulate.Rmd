...
Let's use the `{epiparameter}` package to access and use the available serial interval for MERS disease. For `{epichains}` we need a **discretised** input distribution, se we use `epiparameter::discretise()`:
...
```{r}
# daily aggregate of cases
simulated_chains_day <- multiple_epichains %>%
  # use data.frame output from <epichains> object
  dplyr::as_tibble() %>%
  # transform simulation ID column to factor (categorical variable)
  # dplyr::mutate(chain = as_factor(chain)) %>%
  # get the round number (day) of infection times
