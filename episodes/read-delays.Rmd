...
To do this replacement, instead of plug-in numeric values to `EpiNow2::dist_spec()` to specify the distribution parameters, we are going to collect them from the library of epidemiological parameters provided by `{epiparameter}`:
...
The `epiparameter::epidist_db()` function works as a filtering or subset function. 

We can use other arguments like `author`, `subset` and `single_epidist` to filter even
