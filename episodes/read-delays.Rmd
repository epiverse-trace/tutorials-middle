...
Let's assume that COVID-19 and SARS has similar Reproduction number values. 
...
- COVID-19 has the lowest mean serial interval. The approximate mean value for the serial interval of COVID-19 is around 4 days, and SARS is around 7 days. Thus, COVID-19 is likely to have newer generations in less time than SARS. 
...
Now, instead of using `EpiNow2::dist_spec()` to plug-in numeric values to specify the distribution:
...
Assuming that the disease we are dealing with is COVID, We can replace this code chunk and extract the Serial Interval for COVID-19 using the `epiparameter::epidist_db()` function.
...
We have more than one epidemiological delay. To summarize this view and get the column names from the underlying parameter dataset we connect the previous code chunk with `epiparameter::list_distributions()`:
