...
### From time to probability distributions.

When calculating the *generation time*, we see that not all primary cases have the same probability to generate a secondary case. The onset and cessation of infectiousness may occur gradually.

Equally, not all cases have the same probability to have a time period of equal duration. 

[Serial intervals of possible case pairs in (a) COVID-19 and (b) MERSCoV. Pairs represent a presumed infector and their presumed infectee plotted by date of symptom onset.](fig/serial-interval-pairs.jpg)


[Fitted serial interval distribution for (a) COVID-19 and (b) MERS-CoV based on reported transmission pairs in Saudi Arabia. We fitted three commonly used distribution, lognormal, gamma, and Weibull distributions, respectively.](fig/seria-interval-fitted-distributions.jpg)

For that reason, we refer to all time periods them in terms of their distribution parameters of location (like mean and percentiles)  and spread
...
Table: Serial interval estimates using gamma, Weibull and log normal distributions. 95% confidence intervals for the shape and scale (logmean and sd for log normal) parameters are shown in brackets.
