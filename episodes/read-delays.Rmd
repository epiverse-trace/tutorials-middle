...
We can get the `mean` and standard deviation (`sd`) from this `epidist` we can dive into the `summary_stats` object:
...
Take a minute and explore the nested content with in the `covid_lnorm` using the `$` operator plus the `TAB` button to see them as a list:

```r
covid_lnorm$
```

Share what else you discover from this!
...
Will explore this at the end!

::::::::::::::::::

`EpiNow2::dist_spec()` still needs

The epidemiological parameter in `covid_lnorm` is a continuous distribution:
