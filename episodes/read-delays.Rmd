...
- When to reuse delays from a systematic review?
...
Let's explore how we can access to this and other time delays using `{epiparameter}`. We'll use the pipe `%>%` to connect some of their functions, so let's also call to the `{tidyverse}` package:
...
This is because it is easier to observe and measure the onset of symptoms than the onset of infectiousness.
...
Assume that COVID-19 and SARS have similar reproduction number values and that the serial interval is a fair approximation of the generation time. 
...
- COVID-19 has the lowest mean serial interval. The approximate mean value for the serial interval of COVID-19 is around four days, and SARS is about seven days. Thus, COVID-19 will likely have newer generations in less time than SARS, assuming similar reproduction numbers in both.
...
The objective of the assessment above is to assess on the interpretation of a larger or shorter generation time.
...
Currently, in the library of epidemiological parameters we have one `generation` time entry but for Influenza. Taking the considerations explained above, we can try to look at the `serial` intervals for `COVID`-19.
...
- Then the `⁠<epidist>`⁠ that is parameterised and has the largest sample size will be returned.
...
- What elements you find useful for your analysis?
...
As detailed in `?is_parameterised`, a parameterised distribution is the entry that has a probability distribution associated to it provided by an `inference_method` as shown in `metadata`:
...
With the `day_range` argument you can change the length or number of days in the `x` axis. Explore how does it looks like:
...
- Select a new *Type of distribution*: `Log-Normal`,
- Move the **sliders**, i.e. the graphical control element that allows you to adjust a value by moving a handle along a horizontal track or bar, to the `covid_serialint` parameters. 

Replicate these with the `distribution` object and all its list elements: `2`, `3`, and `4`. Explore How does the shape of a distribution change when its parameters change.

Share about:

- What other features in the website you find useful?
...
In R, all the statistical distributions has functions to access to the:

- Probability **Density** function (PDF),
- Cumulative Distribution function (**CDF**),
- **Quantile** function, and
- **Random** drawn values from the given distribution.
...
- A contact tracing method that consider contacts up to 2 days pre-onset will already capture around 11.1% of backwards cases.

- If this period was extended to 6 days pre-onset then this could include 76.2% of backward contacts.
...
- When considering secondary cases, after how many days following the symptom onset of primary cases can we expect 55% of symptom onset to occur?
...
What is the delay distribution that measures the time between infection and symptoms onset?
...
A severity parameter like the duration of hospitalization could add into the information needed about the bed capacity in response to an outbreak ([Cori et al., 2017](https://royalsocietypublishing.org/doi/10.1098/rstb.2016.0371)).
...
To calculate the Rt we need:

- data set with confirmed cases per day, and
- one key delay distribution

Key functions we applied in this episodes are:
...
For `{EpiNow2}`, we can specify these complementary delay distributions in the `delays` argument.
...
- Add it our last `epinow()` code chunk using the `delays` argument and the `delay_opts()` helper function.
...
There are two complementary delay distributions that we can use to estimate the $R_t$ at time $t$.
...
# subset one distribution for delay of incubation period 
...
# problem
# the summary statistics fo not have mean and sd
...
In the early stage of an outbreak we can rely on reusing parameters, for known and unknown pathogens, even for the [Disease X](https://en.wikipedia.org/wiki/Disease_X), a pathogen currently unknown to cause human disease and potentially cause a  serious international epidemic ([WHO, 2018](https://www.who.int/activities/prioritizing-diseases-for-research-and-development-in-emergency-contexts)). 

But when data from **lines list paired with contact tracing** is available, we are in the possibility to estimate the key delays distributions that best fit our data. This will help us to inform, compare and update any previous estimate about questions like:
...
However, the methods to accurately estimate delays like the generation interval from contact tracing data involve adjusting for biases like like **[censoring](../learners/reference.md#censoring)**, **[right truncation](../learners/reference.md#truncation)** and **[epidemic phase bias](../learners/reference.md#phasebias)**. [(Gostic et al., 2020)](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1008409)
...
### How to estimating delay distributions for Disease X?

Refer to this excellent tutorial on how to estimate the serial interval and incubation period of Disease X accounting for *censoring* using Bayesian inference with packages like `{rstan}` and `{coarseDataTools}`.

- Tutuorial in English: <https://rpubs.com/tracelac/diseaseX> <!-- to request -->
...
- Use `{epiparameter}` to access to systematic review catalog of epidemiological delay distributions.
- Use `epidist_db()` to select single delay distributions.
- Use `list_distributions()` for a overview of multiple delay distributions.
