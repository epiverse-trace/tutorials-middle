...
Let's do this by assigning this `epidist` class object to the `covid_lnorm` object, where `lnorm` means that this epiparameter follows a log-normal distribution.
...
# get the sample_size
covid_lnorm$metadata$sample_size
```
...
```{r}
covid_lnorm$prob_dist
```

This object is class `distribution`. 

```{r}
covid_lnorm$prob_dist %>% class()
```

You can look further into it `?distributional::dist_lognormal()` or the corresponding distribution that you need.

We also recommend your this shinyapp called **The Distribution Zoo**!

<https://ben18785.shinyapps.io/distribution-zoo/>

You can go to the left panel and the second option named `Distribution type`. Select `Log-Normal` and the name of the [sliders](https://shiny.posit.co/r/articles/build/sliders/) will change according to the distribution of your interest.
...
plot(covid_lnorm,day_range = 0:20)
# the density value at quantile value of 10
stats::density(covid_lnorm,at = 10)
# the cumulative probability at quantile value of 10
epiparameter::cdf(covid_lnorm,q = 10)
# the quantile value at a cumulative probability of 50%
stats::quantile(covid_lnorm,p = 0.5)
# generate 10 random values given the distribution family and its parameters
epiparameter::generate(covid_lnorm,times = 10)
```
...
```{r}
stats::quantile(covid_lnorm,p = 0.99)
```
...
One way to do this is getting the quantile value for the the 99th percentile or `0.99` cumulative probability of the distribution. For this we need to access to the set of distribution functions for our `epidist` object.

We can use the set of [distribution functions](https://epiverse-trace.github.io/epiparameter/articles/epiparameter.html#distribution-functions) for a continuous distribution. However, this values will be _continuous_ numbers. To get discrete values from a continuous distribution we can **discretize** the continuous distribution stored in our `epidist` object.
...
To finally get a `max` value let's access to the quantile value of the 99th percentile or `0.99` probability of the distribution with the `prob_dist$q` notation, similarly to how we access to the `summary_stats` values.

```{r}
covid_lnorm_discrete_max <- covid_lnorm_discrete$prob_dist$q(p = 0.99)
```

::::::::::::::::::::::::::::::::: challenge

### 99% of incubation period

Within what time frame do 99% of individuals who develop symptoms exhibit them after infection?

::::::::::::::::: hint

```{r}
plot(covid_lnorm_discrete,day_range = 0:20)
# generate random values
covid_lnorm_discrete$prob_dist$r(1000)
# in what quantile we have the 99.9% cumulative probability?
covid_lnorm_discrete$prob_dist$q(0.99)
# covid_lnorm_discrete$prob_dist$qf(0.99) # as in continuous distribution?
x_limit <- covid_lnorm_discrete$prob_dist$q(0.99)
...
si_discrete_y <- covid_lnorm_discrete$prob_dist$d(si_discrete_x)
...
99% of those who develop symptoms will do so within 16 days of infection.
