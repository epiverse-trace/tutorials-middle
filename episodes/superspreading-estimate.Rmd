...
- **Dispersion**, expressed as $k$, which represents the degree of individual-level variation in transmission by single individuals.
...
- The **individual reproduction number** is a random variable representing the _expected_ number of secondary cases caused by a infected individual.
...
Does the offspring distribution of Ebola is skewed or overdispersed?
...
The negative binomial model has $variance = R_{0}(1+\frac{R_{0}}{k})$, so smaller values of $k$ indicate greater variance and, consequently, greater **individual-level variation**.
...
[Lloyd-Smith et al., 2005](https://www.nature.com/articles/nature04153) use a 20/80 rule to show how the core principle of individual-level variation in transmission is supported.

- When the 20% most infectious cases contribute to the 80% of transmission (or more), there is a high individual-level variation in transmission, with a highly overdispersed offspring distribution ($k<0.3$), e.g., SARS.
...
across_offspring <- epidist_string %>%
  ...
  expand_grid(percent_transmission = seq(0.01, 1, 0.01)) %>%
  # mutate(percent_transmission = 0.8) %>%
  mutate(
    transmission_output =
      pmap(
...
Using `{superspreading}`, we can calculate the probability of having a cluster of secondary infections caused by a primary case identified by backward tracing of size $X$ or larger ([Endo et al., 2020](https://wellcomeopenresearch.org/articles/5-239/v3)).
...
[Lloyd-Smith et al., 2005](https://www.nature.com/articles/nature04153) propose a general protocol for defining a superspreading event (SSE). If the number of secondary infections caused by each case, $Z$, follows a negative binomial distribution ($R, k$):
...
Use `{superspreading}` and compare the estimates for **MERS** using the offspring distributions parameters from this tutorial episode, with **SARS** and **Ebola** offspring distributions parameter accessible via the `{epiparameter}` R package.
