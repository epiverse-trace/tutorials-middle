...
estimates <- EpiNow2::epinow(
  ...
  rt = EpiNow2::rt_opts(prior = base::list(mean = rt_mean, sd = rt_sd))
)
```
...
ebola_incubation_period <- EpiNow2::LogNormal(
  mean = EpiNow2::Normal(mean = 2.487,sd = 0.5),
  sd = EpiNow2::Normal(mean = 0.330, sd = 0.5),
  max = 20
)
...
ebola_cases <- readr::read_csv(file.path("data", "ebola_cases.csv"))
```
...
ebola_estimates <- EpiNow2::epinow(
  data = ebola_cases[1:120, ], # first 3 months of data only
