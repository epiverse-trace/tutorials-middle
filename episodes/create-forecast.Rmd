...
The function `epinow()` described in the [quantifying transmission episode](../episodes/quantify-transmissibility.md) is a wrapper for the function `estimate_infections()` used to estimate cases by date of infection. Let's use the same code used in that episode to get the input data, delays and priors:
...
We can use the `horizon` argument within the `epinow()` function to extend the time period of the forecast.
