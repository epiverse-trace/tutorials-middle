...
Nous supposons ici que la distribution de la descendance du MERS suit une distribution binomiale négative, avec une moyenne (nombre de reproduction $R$) et une dispersion $k$ estimées à partir de la liste de cas et des données de contact de `mers_korea_2015` dans la `{outbreaks}` R dans l'épisode précédent.
...
Utilisons le `{epiparameter}` pour accéder à l'intervalle sériel disponible pour la maladie MERS et l'utiliser :
...
L'intervalle sériel pour le MERS a une moyenne de `r serial_summary$mean` jours et un écart-type de `r serial_summary$sd` jours.
...
Nous sommes maintenant prêts à utiliser la fonction `simulate_chains()` de `{epichains}` pour créer **une** chaîne de transmission :
...
- la distribution de la descendance, et
...
Il s'agit d'une fonction personnalisable de `{epichains}`. Par défaut, les simulations de processus de ramification se terminent lorsqu'elles se sont éteintes. Pour les chaînes de transmission de longue durée, en `simulate_chains()` vous pouvez ajouter le paramètre `stat_threshold`.
...
- `set.seed(<integer>)`, qui est une fonction de générateur de nombres aléatoires avec une valeur de semence spécifiée, le paramètre `<integer>` permet de garantir des résultats cohérents entre les différentes exécutions du code.
...
Les éléments clés de ce résultat se trouvent dans le texte qui apparaît en bas de page :
...
Avant le graphique, créons un tableau récapitulatif avec la durée totale et la taille de chaque chaîne. Nous pouvons utiliser la fonction `{dplyr}` "combo" de `group_by()`, `summarise()` et `ungroup()`:
...
Nous sommes maintenant prêts à utiliser le paquet`{ggplot2}` :
