...
```{.output}
  severity_mean severity_low severity_high
1     0.8082192    0.7583033     0.8517565
```
...
<img src="fig/severity-static-rendered-unnamed-chunk-7-1.png" style="display: block; margin: auto;" />
...
The delay-adjusted CFR indicated that the overall disease severity _at the end of the outbreak_ or with the _latest data available at the moment_ is 0.971 with a 95% confidence interval between 0.852 and 1, slightly higher than the naive one.
...
However, this assumption may not be appropriate for distributions with greater picks. For example, when onset-to-death is strongly peaked with a low variance. In such cases, the average disparity between the PDF and PMF is expected to be more pronounced compared to distributions with broader spreads.

In a scenario like the last one, you can opt to discretise the continuous distribution using `epiparameter::discretise()` to an `<epidist>` object.
...
The `cfr_static()` estimate is useful to get an overall severity estimate of the outbreak (so far). Once the outbreak has clearly ended, or has progressed such that deaths are reported more, the estimated CFR is then _closest to_ the 'true' unbiased CFR.

Additionally, a _delay-adjusted CFR_ estimated with `cfr_static()` can assess the severity of an emerging infectious disease **earlier** than the biased or naive CFR, during an epidemic.

This is particularly useful when an _epidemic curve of confirmed cases_ is the only data available (i.e. when individual data from onset to death are unavailable, especially during the early stage of the epidemic). When there are few deaths or none at all, an assumption has to be made for the *delay distribution* from onset to death, e.g. from literature based on previous outbreaks. [Nishiura et al., 2009](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0006852) depict this in the figures with data from the SARS outbreak in Hong Kong, 2003.

:::::::::::::::::::::::::::::::::: spoiler

### CFR estimation during SARS outbreak, Hong Kong, 2003
...
With `tail()`, we show that the latest CFR estimates are equal to the `cfr_static()` outputs
...
<img src="fig/severity-static-rendered-unnamed-chunk-13-1.png" style="display: block; margin: auto;" />
...
Also, check out the vignette on [Handling data from `{incidence2}`](https://epiverse-trace.github.io/cfr/articles/data_from_incidence2.html)
...
$ newPillarTwoTestsByPublishDate   <dbl> NA, NA, NA, NA, NA, NA, NA, NA, NA, N…
```
