...
When aggregating cases by day with `{incidence2}`, you can adapt the data for `{cfr}` using `cfr::prepare_data()` on `<incidence2>` objects. For details and an example, consult the `{cfr}` reference manual on [preparing common epidemiological data formats for CFR estimation](https://epiverse-trace.github.io/cfr/reference/prepare_data.html).
...
We can use `dplyr::rename()` to adapt the external data to fit the data input for `cfr::cfr_static()`.
...
For each cumulative day $j$ up to $t$, it sums the incident cases $c_i$ over all onset days $i$ up to $j$, weighting each by $f_{j-i}$, the probability of the outcome occurring $j-i$ days after incidence.
...
We can use the `dplyr::rename()` to adapt the external data to fit the data input for `cfr_static()`.
...
First, in the [Get started](https://www.reconverse.org/incidence2/doc/incidence2.html) vignette from the `{incidence2}` package, explore how to use the `date_index` argument when reading a linelist with dates in multiple column.
